...
export default function MergePDFPage() {
    ...
    const [progress, setProgress] = useState(0); // merging progress only
    ...
    const pdfDocRef = useRef<any>(null); // holds pdfjs document
    ...
    const handleFiles = async (fileList: FileList | File[]) => {
        ...
        setStatus('idle'); // immediately idle – no upload spinner
    };
    ...
    /* ───────────────────────── PDF PREVIEW (after merge) ───────────────────────── */
    // load & render on first success render
    useEffect(() => {
        const loadMergedDoc = async () => {
            if (status !== 'done' || !mergedUrl) return;
            ...
            const loadingTask = getDocument(mergedUrl);
        ...
        const renderPage = async (pdf: any, pageNum: number) => {
            ...
            const scale = Math.min(1, targetWidth / viewport.width);
    ...
    /* change page when currentPage changes */
    useEffect(() => {
        const renderIfPossible = async () => {
            if (!pdfDocRef.current || status !== 'done') return;
                    ...
                    <Card className="mx-auto max-w-md border-red-200">
                      ...
                      Page {currentPage} / {pageCount}
                            ...
                            <Button
                                variant="outline"
                                onClick={reset}
                                className="w-full mt-6 text-sm border-red-600 text-red-600 hover:bg-red-50"
                            >
                                merge more files
                            </Button>
                        </CardContent>
